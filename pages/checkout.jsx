      ...
      <div className="relative z-10 w-full flex justify-center items-center flex-col gap-6 px-4">
        ...
        {cart.map((cartItem, index) => {
          return (
            <div
              key={index}
              className="w-full max-w-[750px] bg-white/5 backdrop-blur-xl flex flex-col sm:flex-row min-h-[140px] sm:h-[160px] rounded-3xl shadow-2xl overflow-hidden border border-white/10 group"
            >
              {/* Image */}
              ...
              {/* Middle Section */}
              <div className="w-full sm:flex-1 p-5 sm:p-6 flex flex-col justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-white/40 text-xs tracking-widest uppercase font-bold">
                    ID: {cartItem.product.productID}
                  </p>
                  <h1 className="text-base sm:text-lg font-bold text-white leading-snug line-clamp-2">
                    {cartItem.product.name}
                  </h1>
                </div>

                {/* Quantity Controls */}
                <div className="w-[180px] h-[40px] border border-white/20 rounded-full overflow-hidden flex bg-white/5 shadow-inner">
                  <button
                    onClick={() => {
                      const newCart = [...cart];
              ...
              {/* Price Section */}
              <div className="w-full sm:w-[220px] flex flex-row sm:flex-col justify-between sm:justify-center items-center sm:items-end p-5 sm:p-6 sm:pl-0 text-right shrink-0">
                <div className="text-left sm:text-right mb-0 sm:mb-2">
                  ...
                  <span className="text-sm font-bold text-accent block">
                    {getFormattedPrice(cartItem.product.price)} <span className="text-white/40 text-xs font-normal tracking-wide uppercase">each</span>
                  </span>
                </div>
        ...
        <div className="sticky bottom-6 w-full max-w-[750px] mt-6 z-50">
